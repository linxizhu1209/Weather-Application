...
import { dayTokor, getImage } from '@/composables/helper';
      ...
      <section class="forecast__today">
        ...
        <ul class="week__list">
          ...
          <li v-for="hour in hours" :key="hour.datetime" class="week__list__item">
            <span>{{ dayjs(`2024-08-19 ${hour.datetime}`).format('H') }}시</span>
            <img 
            :src="getImage(hour.icon)" :alt="`${hour.datetime} ${hour.temp}도`" class="week_icons" />
            <span>{{hour.temp}}</span>
          </li>
        </ul>
      </section>
